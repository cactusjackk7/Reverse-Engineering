...
#directions where the PE files that provide the exported functions are located
dll_dirs = ['C:/windows', 'C:/windows/systems32']

# valid extensions of PE files with export directions 
dll_extensions = ['dll']

#directions where the PE files that import are located/
#if the second tuple-member is true, this directory is traversed recursivly/
...
print "Number of input filesL %d" % input_counter
print

counter = 1

for name, hits in stats_list:
	print "% 6d %s" % (counter, hits, name)
	counter = counter + 1
